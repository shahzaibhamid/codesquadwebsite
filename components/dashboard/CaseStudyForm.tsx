  ...
  return (
    <form className="cs-form" action={action} encType="multipart/form-data">
      ...
      <fieldset className="cs-case-form__section">
        ...
        <div className="cs-field"><label htmlFor="case-capabilities">Capabilities</label><textarea id="case-capabilities" name="capabilities" className="cs-textarea cs-textarea--long" defaultValue={caseStudy?.capabilities} placeholder={'Lead operations | Website capture, staff notifications, instant response'} /><small>One group per line: Heading | Comma-separated capabilities. Leave blank if using the icon highlights below instead.</small></div>
        <div className="cs-field"><label htmlFor="case-icon-highlights">Icon highlights (3-icon block)</label><textarea id="case-icon-highlights" name="icon_highlights" className="cs-textarea" defaultValue={caseStudy?.iconHighlights} placeholder={'target | Capture | Every lead across channels\nreply | Respond & Nurture | Automated across 14 days\nchart | Track | Full visibility in one dashboard'} /><small>Up to 3 lines: icon-keyword | Title | Description. Icon keywords match the site icon set (e.g. target, reply, chart, shield, search, bolt).</small></div>
        <div className="cs-field"><label htmlFor="case-before-after">Before / after table</label><textarea id="case-before-after" name="before_after" className="cs-textarea cs-textarea--long" defaultValue={caseStudy?.beforeAfter} placeholder={'Response time | Hours | Under 60 seconds'} /><small>One row per line: Metric | Before | After</small></div>
        ...
        <div className="cs-field"><label htmlFor="case-faq">FAQ</label><textarea id="case-faq" name="faq" className="cs-textarea cs-textarea--long" defaultValue={caseStudy?.faq} placeholder={'Will this replace our booking system? | No — it wraps around your existing system.'} /><small>One item per line: Question | Answer</small></div>
      </fieldset>
