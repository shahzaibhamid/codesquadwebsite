...
function resultsStripSection(c) {
  const stats = c.resultsStrip.stats.map((s) => `<div class="metric reveal"><div class="m-val">${s.val}</div><div class="m-lab">${s.lab}</div></div>`).join('');
  return `
<section class="metrics-band" id="results">
  <div class="wrap">
    <div class="sec-head center reveal" style="margin:0 auto">
      <span class="eyebrow">Real Results</span>
      <h2 class="display">${c.resultsStrip.heading}</h2>
      <p>${c.resultsStrip.sub}</p>
    </div>
    <div class="num-grid">${stats}</div>
    <span class="results-attr reveal">${c.resultsStrip.attribution}</span>
  </div>
</section>`;
}
...
<section class="sec proc" id="process">
  <div class="wrap">
    ...
    <div class="jtimeline">
      ...
      <div class="jrow left"><div class="jnode"></div><div class="jcard">
        <span class="jstep">Step 01</span><h4>Treatment-Based Website</h4>
        <p>Your conversion hub — a dedicated treatment-based landing page built to turn visits into enquiries.</p>
        <div class="jmini">
          <span class="jmb">Website</span>
          <span class="jm-arr"></span>
          <span class="jmb jmb--goal">Treatment-Based Landing Page</span>
        </div>
        <div class="jsample-row"><div class="jsample"><span class="samp-chip">Sample landing page headline</span><p>${firstTreatment.headline}</p></div></div>
      </div></div>

      <div class="jrow right"><div class="jnode"></div><div class="jcard">
        <span class="jstep">Step 02</span><h4>Treatment-Based Google Ad Campaign</h4>
        <p>A dedicated campaign and landing page for every treatment, targeted locally to patients ready to book. Here's one running today:</p>
        <div class="jcard-camp">${sampleCampaignCard(firstTreatment)}</div>
      </div></div>

      <div class="jrow left"><div class="jnode"></div><div class="jcard">
        <span class="jstep">Step 03</span><h4>Leads — Two Routes In</h4>
        <p>Every enquiry is captured the moment it happens, through either route — both feed the next step:</p>
        <svg class="jrsvg" viewBox="0 0 340 160" xmlns="http://www.w3.org/2000/svg">
          <defs><marker id="arL" markerWidth="8" markerHeight="8" refX="5" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z"/></marker></defs>
          <path class="ln" marker-end="url(#arL)" d="M170,42 C170,63 85,57 85,77"/>
          <path class="ln" marker-end="url(#arL)" d="M170,42 C170,63 255,57 255,77"/>
          <path class="ln" d="M85,114 C85,135 170,127 170,141"/>
          <path class="ln" d="M255,114 C255,135 170,127 170,141"/>
          <path class="ln" marker-end="url(#arL)" d="M170,141 L170,152"/>
          <rect class="src" x="123" y="10" width="94" height="32" rx="9"/><text class="st" x="170" y="30">Leads</text>
          <rect class="bx" x="15" y="79" width="140" height="34" rx="9"/><text class="bt" x="85" y="100">Direct Call</text>
          <rect class="bx" x="185" y="79" width="140" height="34" rx="9"/><text class="bt" x="255" y="100">Patient Intake Form</text>
        </svg>
      </div></div>

      <div class="jrow right"><div class="jnode"></div><div class="jcard">
        <span class="jstep">Step 04</span><h4>Potential Patient</h4>
        <p>Both routes create a Potential Patient record straight in your CRM — qualified, tracked and attributed. From here the patient takes one of two paths:</p>
        <svg class="jrsvg" viewBox="0 0 340 160" xmlns="http://www.w3.org/2000/svg">
          <defs><marker id="arP" markerWidth="8" markerHeight="8" refX="5" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z"/></marker></defs>
          <path class="ln" marker-end="url(#arP)" d="M170,42 C170,63 85,57 85,77"/>
          <path class="ln" marker-end="url(#arP)" d="M170,42 C170,63 255,57 255,77"/>
          <path class="ln" d="M85,114 C85,135 170,127 170,141"/>
          <path class="ln" d="M255,114 C255,135 170,127 170,141"/>
          <path class="ln" marker-end="url(#arP)" d="M170,141 L170,152"/>
          <rect class="src" x="105" y="10" width="130" height="32" rx="9"/><text class="st" x="170" y="30">Potential Patient</text>
          <rect class="bx bx--ok" x="15" y="79" width="140" height="34" rx="9"/><text class="bt bt--ok" x="85" y="100">Appointment Booked</text>
          <rect class="bx bx--no" x="185" y="79" width="140" height="34" rx="9"/><text class="bt bt--no" x="255" y="100">Not Booked</text>
        </svg>
      </div></div>

      <div class="jrow left"><div class="jnode"></div><div class="jcard">
        <span class="jstep">Route A</span><h4>Appointment Booked</h4>
        <p>Booked patients flow into your records and stay in the retention loop:</p>
        <div class="jmini">
          <span class="jmb jmb--ok">Appointment Booked</span>
          <span class="jm-arr"></span>
          <span class="jmb">EHR</span>
          <span class="jm-arr"></span>
          <span class="jmb">Dormant Patients</span>
          <span class="jm-arr"></span>
          <span class="jmb jmb--goal">Reactivation</span>
        </div>
        <div class="jsample-row"><div class="jsample"><span class="samp-chip">Sample reactivation message</span><p>${c.flowSamples.reactivation}</p></div></div>
      </div></div>

      <div class="jrow right"><div class="jnode"></div><div class="jcard">
        <span class="jstep">Route B</span><h4>Not Booked → Automated Nurture</h4>
        <p>Anyone who doesn't book is nurtured until they do — then it stops:</p>
        <div class="jmini">
          <span class="jmb jmb--no">Appointment Not Booked</span>
          <span class="jm-arr"></span>
          <span class="jmb">SMS / Email Campaign</span>
          <span class="jm-arr"></span>
          <span class="jmb">Follow-up Campaign</span>
        </div>
        <span class="jloop-note">↺ Loops back to Potential Patient</span>
        <div class="jsample-row">
          <div class="jsample"><span class="samp-chip">Sample nurture SMS</span><p>Hey [First Name], {Person Name} with {Clinic Name} here. This {Month}, new clients get ${c.messaging.variants[0].featuredOffer}. Reserve: {Booking Link}</p></div>
          <div class="jsample"><span class="samp-chip">Sample nurture email</span><p><em>Subject:</em>[First Name], here's what your first visit at {Clinic Name} looks like</p></div>
        </div>
      </div></div>

      <div class="jrow left"><div class="jnode"></div><div class="jcard">
        <span class="jstep">Parallel Engine</span><h4>Organic / SEO Engine</h4>
        <p>A second engine feeding the same funnel, compounding month after month:</p>
        <div class="jmini">
          <span class="jmb">SEO · GEO · AEO</span>
          <span class="jm-arr"></span>
          <span class="jmb">Treatment / Service Content</span>
          <span class="jm-arr"></span>
          <span class="jmb">Blogs</span>
          <span class="jm-arr"></span>
          <span class="jmb">Social Media + GBP Posts</span>
          <span class="jm-arr"></span>
          <span class="jmb jmb--goal">Potential Patients</span>
        </div>
        <div class="jsample-row">
          <div class="jsample"><span class="samp-chip">Sample blog · SEO / AEO</span><h5>${c.flowSamples.blog.title}</h5><p>${c.flowSamples.blog.meta}</p></div>
          <div class="jsample"><span class="samp-chip">Sample GBP post</span><p>${c.flowSamples.gbpPost}</p></div>
        </div>
      </div></div>

      <div class="jrow right"><div class="jnode"></div><div class="jcard">
        <span class="jstep">The Hub</span><h4>Customized CRM</h4>
        <p>The layer connecting every step above — one dashboard your whole team logs into.</p>
        <div class="jpills"><span class="jpill">Ad Analytics</span><span class="jpill">Leads Data</span><span class="jpill">Campaign Analytics</span><span class="jpill">Business Profile</span><span class="jpill">UTM Tracking</span><span class="jpill">Automation Controller</span><span class="jpill">Team Logins</span></div>
      </div></div>
      ...
      <div class="jrow left"><div class="jnode"></div><div class="jcard">
        ...
        <div class="jmini">
          <span class="jmb">$1,000/mo ad spend</span>
          <span class="jm-arr"></span>
          <span class="jmb">40–50 leads / month</span>
          <span class="jm-arr"></span>
          <span class="jmb">25–30 potential patients</span>
          <span class="jm-arr"></span>
          <span class="jmb jmb--goal">Closure by the practice</span>
        </div>
      </div></div>
    </div>
  </div>
...
function campaignTypeCard(t) {
  return `<div class="camp-type reveal">
    <span class="camp-type-tag">${t.tag}</span>
    <h4>${t.name}</h4>
    <p class="camp-type-goal">${t.goal}</p>
    <div class="camp-type-meta">
      <span><b>Targeting</b>${t.targeting}</span>
      <span><b>Lands on</b>${t.landsOn}</span>
    </div>
  </div>`;
}
...
function campaignsSection(c) {
  const typeCards = CAMPAIGN_TYPES.map(campaignTypeCard).join('');
  ...
  const groups = c.campaigns.groups
    .map((g, i) => `<div class="camp-sample-grid"${g.key ? ` data-toggle-panel="${g.key}"${i === 0 ? '' : ' style="display:none"'}` : ''}>${g.samples.map(sampleCampaignCard).join('')}</div>`)
    .join('');
...
<section class="sec creambg" id="campaigns">
  <div class="wrap">
    ...
    <div class="camp-type-grid">${typeCards}</div>
    ${toggle}
    ${groups}
  </div>
</section>`;
  ...
  const panels = c.messaging.variants
    .map((v, i) => {
      ...
      return `<div class="msg-panel"${hasToggle ? ` data-toggle-panel="${v.key}"${i === 0 ? '' : ' style="display:none"'}` : ''}>
        ...
        <div class="msg-screenshot reveal"><img src="/images/patient-growth/sms-instant-reply.png" alt="A real instant SMS reply sent automatically from the CRM" loading="lazy"/></div>
  ...
  <div class="wrap">
    ...
    <div class="svc-grid svc-grid--compact">
      <div class="svc reveal">
        ...
        <div class="svc-body">
          <h3>Google Ads → CRM → Booking</h3>
          <p>Treatment-based campaigns, landing pages, lead capture, instant follow-up, 14-day nurture and booking — end to end.</p>
          <ul>
            <li>A separate campaign per treatment — weight loss, injectables, skin — each with its own budget and page</li>
            <li>Local, intent targeting with 44+ negative keywords blocking wasted spend</li>
            <li>Full attribution captured automatically — GCLID, campaign, ad group, keyword</li>
            <li>Conversions fire only after a lead is confirmed, so reporting is never inflated</li>
          </ul>
        </div>
      </div>
      <div class="svc reveal">
        ...
        <div class="svc-body">
          ...
          <ul>
            <li>Written around what patients search — treatment comparisons, timelines, guides</li>
            <li>Technical SEO prepared automatically from the primary keyword</li>
            <li>Structured to be quoted by AI search and answer engines, not just Google</li>
            <li>Manual control — nothing publishes without an editor clicking confirm</li>
          </ul>
        </div>
      </div>
      <div class="svc reveal">
        ...
        <div class="svc-body">
          ...
          <ul>
            <li>Leads, statuses, message history and campaign enrollment in one view</li>
            <li>Google Ads workspace, website analytics and lead events in the same place</li>
            <li>Automated health checks on delivery, enrollment and campaign timing</li>
            <li>Role-based staff access and a full audit log of every change</li>
          </ul>
        </div>
      </div>
      <div class="svc reveal">
        ...
        <div class="svc-body">
          ...
          <ul>
            <li>Reviews pulled into the dashboard so nothing sits unanswered</li>
            <li>Draft replies prepared for staff to approve and post</li>
            <li>Profile posts published to keep the listing active</li>
            <li>Local search insights reported alongside campaign and website data</li>
          </ul>
        </div>
      </div>
    </div>
...
<section class="sec creambg" id="case">
  <div class="wrap case">
    <div class="reveal">
      ...
      <div class="num-grid" style="margin:22px 0 26px">${stats}</div>
      <p>${cs.intro}</p>
...
<section class="sec">
  <div class="wrap">
    <div class="reveal">
      ...
      <h2 class="display" style="font-size:clamp(26px,3.6vw,40px);margin:12px 0 26px;font-weight:900;letter-spacing:-.03em">Everything connected. Nothing siloed.</h2>
      <div class="stack-tags">
        <span class="st">Google Ads</span><span class="st">Custom website &amp; landing pages</span><span class="st">Automation engine</span><span class="st">Operational database</span><span class="st">Transactional SMS &amp; email</span><span class="st">Your booking system</span><span class="st">Custom CRM dashboard</span><span class="st">Google Analytics</span><span class="st">Google Business Profile</span>
      </div>
    </div>
  </div>
</section>`;
...
function pricingSection() {
  return `
<!-- OFFERS -->
<section class="sec creambg" id="pricing">
  <div class="wrap">
    <div class="plan-wrap plan-wrap--offers reveal">
      <div class="plan plan--featured">
        <div class="plan-badge">Our offers include</div>
        <p class="plan-sub">Everything your clinic needs to stop losing leads and scale patient growth — all in one place.</p>
        <ul class="plan-list plan-list--three">
        ...
        <a href="#contact" class="btn btn-dark plan-cta">Get started →</a>
